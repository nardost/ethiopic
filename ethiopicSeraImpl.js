...
var token = null;
var output;
var oflag; //if true, update last character of output instead of appending to the output.
var pflag; //true for punctuation marks. add space before and after the punctuation mark.
...
//Preserve default behavior for some keys.
...
function test(e) {
	...
	alert('event.code = ' + e.code + '\nevent.key = ' + e.key + '\nCode to Key: ' + codeToKey(e.code, true, false)); //CapsLock, e.shiftKey = true | false
	
	if(aeiou.indexOf(s) != -1) {
		seleda.innerHTML += String.fromCharCode(a(s)) + '\t' + '<br />';
	} else if(lambda.indexOf(s) != -1) {
	...
	seleda.scrollTop = seleda.scrollHeight; //scroll automatically as input grows.
}
//repeated action.
...
function transliterate(event) {
	...
	var capsLockOn = false; //do CapsLock detection later.
	...
	/**Some keys retain their default behavior*/
	if(defaultBehaviorKeys.indexOf(k) != -1) {
		/**
		 * Backspace, Delete & Arrow keys lead to state = 0.
		 * */
		if(['Backspace', 'Delete'].indexOf(event.code) != -1) {
			doTransition(0, null, null, false);
		} else if(['ArrowLeft', 'ArrowUp', 'ArrowRight', 'ArrowDown'].indexOf(event.code) != -1) {
			doTransition(0, null, null, false);
		}
		return true;
	}
	
	/** ctrl + c, ctrl + v, etc should be possible.*/
	...
	switch(k) {
		...
		case ' ':
			...
			break;
		default:
			if(['I', 'O', 'U'].indexOf(k) != -1) return true; //for now, not used in the transliteration.
			...
			if(state == 1) {
				if(aeiou.indexOf(token) != -1) {
					doTransition(1, k, sadis(k), false);
				} else if((alpha.concat(beta)).indexOf(token) != -1) {
					if(k == 'W') {
				...
				} else if(fia.indexOf(token) != -1) {
					if(k == 'Y') {
						doTransition(0, null, sadis(token+k), true); //so far the only case where state goes to 0 on sadis.
					} else {
						if(k == 'W') {
							doTransition(0, null, z_rabee(token), true);
						} else {
				...
				} else if((gamma.concat(delta)).indexOf(token) != -1) {
	...
	if(start == end) {
		if(editor.value.length == end) {
			if(oflag) editor.value = editor.value.slice(0, editor.value.length - 1); //overwrite the last character.
			editor.value += String.fromCharCode(output);
		} else {
			if(oflag) {
				editor.value = editor.value.substring(0, start - 1) + editor.value.substring(start, editor.value.length); //overwrite the last character.
				//editor.setSelectionRange(start-1, start-1);
			}
			editor.value = editor.value.substring(0, start) + String.fromCharCode(output) + editor.value.substring(end, editor.value.length);
			//editor.setSelectionRange(start+1, start+1);
		}
	} else {//some text selected
		editor.value = editor.value.substring(0, start) + String.fromCharCode(output) + editor.value.substring(end, editor.value.length);
	}
	if(pflag) editor.value += ' ';
	return false;
}
/**generates አ ኡ ኢ ኣ ኤ እ ኦ ኧ ዐ ዑ ዒ ዓ ዔ ዕ ዖ*/
function a(x) {
	switch(x) {
...
/**functions that generate geez, kaeeb, salis, etc...*/
...
function z_rabee(consonant) {
	if(lambda.indexOf(consonant) != -1) {
		//irregular zemede-rabee.
		var z = ['g', 'h', 'hh', 'k', 'K', 'q', 'Q', 'w', 'W', 'y', 'Y', 'gW', 'kW', 'KW' , 'QW', 'SS'];//G is in this list in the wikimedia transliterator.
		...
		switch(consonant) {
			...
			case 'Y': 
			case 'gW':
			//case 'G' : Wikimedia transliterator does not implement z_rabee('G'). I have yet to find out.
			case 'kW':
			case 'KW':
			case 'QW':
			case 'SS': return null;
